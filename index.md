...
Sprint 評審會議是每個 Sprint 最後倒數的第二個事件，是有時間盒限定的：一個月的 Sprint 最多為四個小時；而較短的 Sprint，這個事件所需時間通常會更短。
...
凡是 Scrum 團隊能夠在一個 Sprint 中「完成」(Done) 的產品待辦清單項目 ，就可視為是備妥的，而可在 Sprint 計劃會議中被挑選。通常在精煉活動後才可達到這樣的透明性。產品待辦清單精煉是將產品待辦清單項目拆解並進一步定義，使其變得更小、更精確的活動。這是一項持續進行的活動，加入更多描述、順序與大小之類的細節。這些屬性通常隨工作領域而不同。
